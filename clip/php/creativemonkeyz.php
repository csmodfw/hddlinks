...
<channel>
	<title><?php echo urldecode($tit); ?></title>
	<menu>main menu</menu>
...
foreach($videos as $video) {
   ...
   for ($i=0;$i<count($links);$i++) {
    ...
    if (strpos($cur_link,"player.vimeo.com") !== false) {
      ...
      $n=$n+1;
      $title1 = $title." (".$n.") - vimeo";
      $link1 = 'http://127.0.0.1/cgi-bin/scripts/clip/php/roboti_link1.php?file='.urlencode($cur_link).",".urlencode($link);
    } else if(preg_match('/youtube\.com\/(v\/|watch\?v=|embed\/)([\w\-]+)/', $cur_link)) {
      $n=$n+1;
      $title1 = $title." (".$n.") - youtube";
      $link1="http://127.0.0.1/cgi-bin/scripts/util/yt.php?file=".$cur_link;
    }
    $name = preg_replace('/[^A-Za-z0-9_]/','_',$title1).".mp4";
    if ($link1 <> "") {
	echo'
	<item>
	<title>'.$title1.'</title>
