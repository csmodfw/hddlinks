...
if ($page==1) {
  $html = file_get_contents("http://creativemonkeyz.com/category/roboti/");
} else {
  $html = file_get_contents("http://creativemonkeyz.com/category/roboti/page/".$page."/");
}
if($page > 1) { ?>
...
foreach($videos as $video) {
    ...
    $link = 'http://127.0.0.1/cgi-bin/scripts/clip/php/roboti_link.php?file='.urlencode($link);

    $name = preg_replace('/[^A-Za-z0-9_]/','_',$title).".mp4";
    if (strpos($title,"Goodies") === false) {
	echo'
	<item>
	<title>'.$title.'</title>
    <onClick>
