...
$l="http://www.tjoob.com/categories/all/top/";
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $l);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION  ,1);
  curl_setopt($ch, CURLOPT_REFERER, "http://www.tjoob.com");
  $html = curl_exec($ch);
  curl_close($ch);
...
foreach($videos as $video) {
    ...
    $t1=explode('"',$t[1]);
    $link="http://www.tjoob.com".$t1[0]."top-";
  	$title=str_between($video,'<strong>','</strong>');
    $link=$host."/scripts/adult/php/tjoob.php?query=1,".$link;
    echo '
    <item>
    <title>'.$title.'</title>
    <link>'.$link.'</link>
