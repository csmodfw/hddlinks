...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/adult/php/4tube.php?query=1,"; ?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<channel>
	<title>4tube</title>
...
foreach($videos as $video) {
    ...
    $t3=explode('"',$t2[1]);
  	...
  	$link=$host."/scripts/adult/php/4tube.php?query=1,".$link.",release";
  	echo '
  	<item>
  		<title>'.$title.'</title>
  		<link>'.$link.'</link>
...
$l="http://www.4tubes.com/channels";
  ...
  curl_close($ch);
//$html = str_between($html,'Categories','</ul>');
$videos = explode('class="thumb-link', $html);
unset($videos[0]);
$videos = array_values($videos);
foreach($videos as $video) {
    $t=explode('href="',$video);
