...
$html=file_get_contents($link);
    $t1=explode('img src="',$html);
    $t2=explode('?',$t1[1]);
    $image=str_replace("https","http",$t2[0]);
$html=str_between($html,'<code>','</code>');
//echo urlencode($html);
//echo $html;

if (strpos($html,'#EXTINF:-1') !== false) {
$r = explode(urldecode("%0D%0A"), $html);
//print_r($r);
for ($k=0;$k<count($r);$k++) {
...
for ($k=0;$k<count($r);$k++) {
  if ($r[$k] <> "") {
    ...
    $title="playlist ".$p;
    $p++;
    $link = 'http://127.0.0.1/cgi-bin/scripts/tv/php/playlist.php?query='.urlencode($title).','.urlencode($l);
    echo '
    <item>
