...
<?php
$page = $_GET["page"];
//http://iptvsatlinks.com/
if($page > 1) {
 $l="https://www.dailyiptvlist.com/page/".$page."/";
} else {
  $page = 1;
  $l="https://www.dailyiptvlist.com/";
}
...
$videos = explode('class="cat-title', $html);
...
foreach($videos as $video) {
    ...
    $t2=explode('"',$t1[2]);
    $link=$t2[0];
    $t3=explode('title="',$t1[2]);
    $t2=explode('"',$t3[1]);
    $title=trim($t2[0]);
    $t1=explode('src="',$video);
    $t2=explode('"',$t1[1]);
    $image=str_replace("https","http",$t2[0]);

	if ($title <> ""){
    $link = 'http://127.0.0.1/cgi-bin/scripts/tv/php/dailyiptvlist.php?file='.urlencode($link).','.urlencode($title);
    echo '
    <item>
