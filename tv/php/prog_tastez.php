...
$id = $_GET["file"];
$id=str_replace("protv","pro-tv",$id);
//if (substr($id, -2) == "ro" || substr($id, -2) == "tv") $id=trim(substr($id, 0, -2));
$n=0;
...
$l="http://tastez.ro/program.php";
  ...
  curl_close($ch);
//echo $html;
$videos = explode("program.php", $html);
...
foreach($videos as $video) {
  ...
  $title="";
  $t1=explode('"',$video);
  $link="http://tastez.ro//program.php".$t1[0];
  $link=str_replace("&amp;","&",$link);
  $t2=explode(">",$video);
  $t3=explode("<",$t2[1]);
  $title=strtolower(str_replace(" ","-",$t3[0]));
  //if (stripos($title, $id) !== false) break;
  if ($title==$id) break;
}
//echo $link;
if ($title) {
