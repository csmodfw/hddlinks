...
if (userInput == "pagedown" || userInput == "pageup")
{
  ...
  "true";
	...
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/sport/sportro_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
//$html = file_get_contents("http://www.sport.ro/video/sporturi/pagina-".$page."/");
$l="https://www.sport.ro/video/sporturi/pagina-".$page."/";
/*
  $ch = curl_init();
...
//echo $html;
$videos = explode('li class="cf"', $html);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $image = "http://www.sport.ro".$t2[0];
    //"http://www.sport.ro".
    $t1 = explode('href="', $video);
    $t4 = explode('"',$t1[1]);
    $link="https://www.sport.ro".$t4[0];
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/sport/sportro_link.php?file='.urlencode($link).'";
    movie=getUrl(url);
    cancelIdle();
    playItemUrl(movie,10);
    </script>
    </onClick>
    <download>'.urlencode($link).'</download>
    <name>'.$name.'</name>
