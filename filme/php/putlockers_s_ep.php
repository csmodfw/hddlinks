...
//$cookie="D:/";
  ...
  //echo $html;
  $link1="";
 $videos = explode('td width="100%"', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $t2=explode('"',$t1[1]);
  $link1=$t2[0];
  $ep_tit=trim(str_between($video,'</strong></a>','<'));
  ...
  $episod=$m[2];
   ...
   $title=$ep_tit;
   $image1=$image;
   //$link2=$host."/scripts/filme/php/putlockers_link.php?file=".urlencode($link1).",".urlencode($title1).",".urlencode($series_title_no_year).",".$id_t.",series,".urlencode($image);
   $link2=$host."/scripts/filme/php/putlockers_link.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title1)).",".urlencode(str_replace(",","^",$series_title_no_year)).",".$id_t.",series,".urlencode($image);
   if ($video) {
     echo '
     <item>
     <title>'.$title.'</title>
