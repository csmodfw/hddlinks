...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/pix_main.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
<?php
if ($tip=="search")
   $requestLink = "https://topeuropix.net/search.php?search=".str_replace(" ","+",$link);
else
   $requestLink="https://topeuropix.net/tvshow-filter/all-tv-shows-page-".$page;
...
foreach($videos as $video) {
  ...
  $t2 = explode("'", $t1[1]);
  $link1 = $t2[0];
  if (strpos($link1,"../") !== false) $link1="https://topeuropix.net/".str_replace("../","",$link1);

  $t1 = explode("opisser'><h3>", $video);
  $t2 = explode('<', $t1[1]);
  $title11 = trim($t2[0]);
  ...
  $id_t="";
   ...
   if (strpos($link1,"/tv") !== false || strpos($link1,"/zz-tvshow") !== false) {
     echo '
     <item>
...
foreach($videos as $video) {
  ...
  $t1 = explode("id='opisser'><h3>", $video);
  $t2 = explode('<', $t1[1]);
  $title11 = trim($t2[0]);
  $title11=trim(preg_replace("/Watch|Putlocker/i","",$title11));
  ...
  $id_t="";
   $link2=$host."/scripts/filme/php/pix_sez.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
   //if (strpos($link1,"/tvser") !== false || strpos($link1,"/zz-tvshow") !== false) {
     echo '
     <item>
     <title>'.str_replace("&","&amp;",str_replace("&amp;","&",$title)).'</title>
     <link>'.$link2.'</link>
