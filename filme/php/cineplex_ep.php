...
$sez="1";
while (true) {
  $l="https://".$server."/series/season?id=".$id."&s=".$sez."&token=".$token."&_";
...
foreach ($episoade as $key => $value) {
  ...
  $se=$season."x".$episod;
   if ($title) {
     $l1="http://127.0.0.1/cgi-bin/scripts/filme/php/cineplex_link.php?file=series,".$imdb.",".$token.",".urlencode(str_replace(",","^",$page_title)).",".urlencode($image).",".$se.",".urlencode(str_replace(",","^",$title));
     //$l1="http://127.0.0.1/cgi-bin/scripts/filme/php/royale_link.php?file=series,".$imdb.",".$ep_nr;
     echo '
     <item>
     <title>'.str_replace("&","&amp;",str_replace("&amp;","&",$title)).'</title>
     <link>'.$l1.'</link>
    <image>'.$img_ep.'</image>
    <tit>'.urlencode(str_replace(",","^",$page_title)).'</tit>
    <an>'.$year.'</an>
    <movie>'.$imdb.'</movie>
     </item>
     ';
