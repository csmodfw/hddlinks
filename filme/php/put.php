...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/put_add.php?mod=add," + urlEncode(movie) + "," + tit + "," + urlEncode(img) + "," + urlEncode(year) + "," + urlEncode(id);
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/put.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/put_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
if ($tip=="search")
   $requestLink = "http://putlocker.tl/search-movies/".str_replace("&","",str_replace(" ","+",$link))."/page-".$page.".html";
   else {
   if($page>1) {
  $requestLink ="http://putlocker.tl/tv-series/page-".$page.".html";
} else {
	$page = 1;
  $requestLink="http://putlocker.tl/tv-series.html";
}}
//http://putlocker.tl/search-movies/lois+%26+clark.html
//http://putlocker.tl/search-movies/lois/page-2.html
...
//$cookie="D:/";
  ...
  curl_setopt ($ch, CURLOPT_REFERER, "http://putlocker.tl");
...
foreach($videos as $video) {
  ...
  if (preg_match("/-season-/i",$link1))
   $link2=$host."/scripts/filme/php/put_ep.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
  else
   $link2=$host."/scripts/filme/php/put_link.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",movie,".urlencode($image);
