...
year=getItemInfo(getFocusItemIndex(),"an");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/cineplex_add.php?mod=add," + urlEncode(movie) + "," + urlEncode(tit) + "," + urlEncode(img) + "," + urlEncode(year);
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
showIdle();
movie_info="http://127.0.0.1/cgi-bin/scripts/filme/php/tmdb.php?query=movie," + tit+ "," + an;
...
if ($tip=="release") {
  ...
  $post="loadmovies=showData&page=".$page."&abc=All&genres=".$gen."&sortby=Recent&quality=All&type=movie&q=&token=".$token;
...
//echo $html;
  ...
  foreach($videos as $video) {
    ...
    $imdb=$t2[0];
   if ($title) {
     $l1="http://127.0.0.1/cgi-bin/scripts/filme/php/cineplex_link.php?file=movie,".$imdb.",".$token.",".urlencode(str_replace(",","^",$title)).",".urlencode($image);
     echo '
     <item>
...
$post="loadmovies=showData&page=1&abc=All&genres=&sortby=Recent&quality=All&type=movie&q=".str_replace("+","%20",urlencode($link))."&token=".$token;
$ua="Mozilla/5.0 (Windows NT 10.0; rv:55.0) Gecko/20100101 Firefox/55.0";
$exec_path="/usr/local/bin/Resource/www/cgi-bin/scripts/wget ";
