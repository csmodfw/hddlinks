...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/putlockerfit_add.php?mod=add," + urlEncode(movie) + "," + tit + "," + urlEncode(img) + "," + urlEncode(year) + "," + urlEncode(id);
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
tit=getItemInfo(getFocusItemIndex(),"tit");
showIdle();
movie_info="http://127.0.0.1/cgi-bin/scripts/filme/php/putlockerfit_det.php?file=" + movie+ "," + tit;
...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/putlockerfit.php?page=2,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/putlockerfit_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
if ($tip=="search") {
   $requestLink = "https://www1.putlockerfit.net/?s=".str_replace(" ","+",$link);
   $requestLink = "https://www2.putlockerfit.net/?s=".str_replace(" ","+",$link);
//echo $requestLink;
...
/*
  ...
  curl_setopt ($ch, CURLOPT_REFERER, "https://putlockerfit.net/");
...
foreach($videos as $video) {
  ...
  //$id_t=$id1;
  $id_t="";
   $link2=$host."/scripts/filme/php/putlockerfit_ep.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
   if ($title && strpos($link1,"show/") !== false) {
     echo '
     <item>
...
$host1=$r["host"];
  ...
  curl_close ($ch);
  */
 $videos = explode('<td><a class="', $html);
unset($videos[0]);
$videos = array_values($videos);
foreach($videos as $video) {
  $t1=explode('href="',$video);
  $t2=explode('"',$t1[1]);
  $link1=$t2[0];
  if (strpos($link1,"http") === false) $link1="https://".$host1.$link1;
  $t3=explode(">",$t1[1]);
  $t4=explode('<',$t3[1]);
  $title11=$t4[0];
  $title11=str_replace("#038;","",$title11);
