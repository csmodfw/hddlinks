...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/putlocker_f.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
if ($tip=="search")
   $requestLink = "http://www.watchfree.to/?keyword=".str_replace(" ","+",$link)."&search_section=1&page=".$page;
else
   $requestLink = "http://www.watchfree.to/?sort=date&keyword=&page=".$page."";
...
foreach($videos as $video) {
   ...
   */
   ...
   $link2=$host."/scripts/filme/php/putlocker_link.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".urlencode(str_replace(",","^",$series_title_no_year)).",".$id_t.",movie,".urlencode($image);
   if ($video) {
     echo '
     <item>
     <title>'.str_replace("&","&amp;",str_replace("&amp;","&",$title)).'</title>
