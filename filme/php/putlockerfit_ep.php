...
$series_title=str_replace("#038;","",$series_title);
...
//$cookie="D:/";
 ...
 $videos = explode('class="btn-episode', $html);
unset($videos[0]);
...
foreach($videos as $video) {
    ...
    $num=str_between($video,'strong>','<');
    $ep_tit=trim(preg_replace("/".$series_title."/","",$ep_tit));
    $ep_tit=trim(preg_replace("/season\s+(\d+)\s+episode\s+(\d+)/i","",$ep_tit));
  //$t1=explode('>',$video);
  //season-1-episode-3
  //Star Trek: Enterprise Season 4 Episode 22
  preg_match("/season\s+(\d+)\s+episode\s+(\d+)/i",$num,$m);
  //print_r ($m);
  $season=$m[1];
  $episod=$m[2];
   ...
   //$image=$t2[0];
   $image1=$image;
   $link2=$host."/scripts/filme/php/putlockerfit_link.php?file=".urlencode($link_ep).",".urlencode(str_replace(",","^",$title1)).",".urlencode(str_replace(",","^",$series_title_no_year)).",".$id_t.",series,".urlencode($image);
   if ($video) {
     echo '
     <item>
