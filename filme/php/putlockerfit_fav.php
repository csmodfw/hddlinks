...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/putlockerfit_add.php?mod=del," + urlEncode(movie) + ",,,,";
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>putlockerfit - favorite</title>
...
if (file_exists("/data"))
  $f= "/data/putlockerfit.dat";
else
  $f="/usr/local/etc/putlockerfit.dat";
...
foreach($videos as $video) {
  ...
  $t2=explode("<",$t1[1]);
  $image=$t2[0];
  if (strpos($image,"https") === false) $image=str_replace("http","https",$image);
...
foreach ($arr as $key => $val) {
  ...
  $image1=$image;
   $link2=$host."/scripts/filme/php/putlockerfit_ep.php?file=".urlencode($link).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
