...
if ($page==1)
  $l="https://desene-super-ade3.blogspot.ro/search/label/".$page_title;
else
  $l="https://desene-super-ade3.blogspot.ro/search/label/".$page_title."?updated-max=".urlencode($search); //."&max-results=20&start=12&by-date=false";
...
foreach($videos as $video) {
  ...
  $image=str_replace("https","http",$image);
	if($link!="") {
		...
		$link = "http://127.0.0.1/cgi-bin/scripts/filme/php/deseneledublate2_ep.php?file=".$link.",".urlencode($title);
		echo'
		<item>
		<title>'.$title.'</title>
		<link>'.$link.'</link> 
