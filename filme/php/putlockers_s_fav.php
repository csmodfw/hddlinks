...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/putlockers_s_add.php?mod=del," + urlEncode(movie) + ",,,,";
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>putlockers - favorite</title>
...
if (file_exists("/data"))
  $f= "/data/putlockers_s.dat";
else
  $f="/usr/local/etc/putlockers_s.dat";
if (file_exists($f)) {
...
foreach ($arr as $key => $val) {
  ...
  $image1=$image;
   $link2=$host."/scripts/filme/php/putlockers_s_ep.php?file=".urlencode($link).",".urlencode($title).",".$id1.",".$id_t.",series,".urlencode($image);
