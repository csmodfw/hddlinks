...
$base=str_replace(substr(strrchr($link, "/"), 1),"",$link);
$imdb="";
$ep=$id_t;
//echo $base;
//die();
...
foreach($videos as $video) {
  ...
  $l1=$base.$t2[0];
      $ua="Mozilla/5.0 (Windows NT 5.1; rv:52.0) Gecko/20100101 Firefox/52.0";
      $exec = '-q -S -U "'.$ua.'" --referer="'.$li.'" --no-check-certificate "'.$l1.'" -O - 2>&1';
      $exec = "/usr/local/bin/Resource/www/cgi-bin/scripts/wget ".$exec;
      $h=shell_exec($exec);
      //echo $h;
  ...
  //echo $link_ep."\n";
  if (strpos($link_ep,"hdeuropix") !== false) {
      $ua="Mozilla/5.0 (Windows NT 5.1; rv:52.0) Gecko/20100101 Firefox/52.0";
      $exec = '-q -S -U "'.$ua.'" --referer="'.$link_ep.'" --no-check-certificate "'.$link_ep.'" -O - 2>&1';
      $exec = "/usr/local/bin/Resource/www/cgi-bin/scripts/wget ".$exec;
      $h1=shell_exec($exec);
    if (strpos($h1,"404 Not Found") === false) {
    $link_ep="";
    $link_ep=str_between($h1,"iframe src='","'");
    if ($link_ep) $r[]=$link_ep;
    }
  } else {
