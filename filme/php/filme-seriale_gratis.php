...
include("../../common.php");
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$t1=explode('itemprop="image" src="',$html);
//$t2=explode('src="',$t1[1]);
$t3=explode('"',$t1[1]);
$img=$t3[0];
//$img="http://127.0.0.1/cgi-bin/scripts/filme/php/r_wget.php?file=".urlencode($img);
$img="http://127.0.0.1/cgi-bin/scripts/util/wget.cgi?link=".$img;
    $data = trim(str_between($html,'<p><strong>','<span'));
