...
year=getItemInfo(getFocusItemIndex(),"an");
 ...
 <?php
 if ($tip=="movie")
 echo 'url="http://127.0.0.1/cgi-bin/scripts/filme/php/royale_add.php?mod=add," + urlEncode(movie) + "," + urlEncode(tit) + "," + urlEncode(img) + "," + urlEncode(year);';
 else
 echo ' url="http://127.0.0.1/cgi-bin/scripts/filme/php/royales_add.php?mod=add," + urlEncode(movie) + "," + urlEncode(tit) + "," + urlEncode(img) + "," + urlEncode(year);';
?>
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
...
showIdle();
movie_info="http://127.0.0.1/cgi-bin/scripts/filme/php/royale_det.php?file=" + tip + "," + movie+ "," + urlEncode(tit) + "," + img;
...
$l="https://".$server."/api/v1/person/".$id;
...
if ($tip=="movie") {
echo '
	<title>Filme in care joaca '.$name.'</title>
	<menu>main menu</menu>
	';
} else {
echo '
    <title>Seriale in care joaca '.$name.'</title>
	<menu>main menu</menu>
	';
}
...
$c=count($r["titles"]);
for ($k=0;$k<$c;$k++) {
  $title= $r["titles"][$k]["title"];
  $year = $r["titles"][$k]["year"];
  $image= "http://image.tmdb.org/t/p/w300".$r["titles"][$k]["poster"];
  $pm=$r["titles"][$k]["permalink"];
  preg_match("#tt\d+#",$pm,$m);
  $imdb=$m[0];
  $char=$r["titles"][$k]["char"];
  $title1=$title." .. as ".$char;
   if(strpos($pm,"/movies/") !== false) {
