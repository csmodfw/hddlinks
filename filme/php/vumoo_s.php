...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/vumoo_s.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/vumoo_s_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
foreach($videos as $video) {
  ...
  $t2 = explode('"', $t1[1]);
  $title11 = $t2[0];
  $id1=str_between($video,'movie_info/','"');
  ...
  //$id_t=$id1;
  $id_t="";
   $link2=$host."/scripts/filme/php/vumoo_s_ep.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
   if ($title) {
     echo '
     <item>
     <title>'.$title.'</title>
