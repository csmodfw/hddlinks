...
>
      ...
      1=add to favorite, right for more
		</text>
...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/gomovies_s_add.php?mod=add," + urlEncode(movie) + "," + urlEncode(tit) + "," + urlEncode(img) + "," + urlEncode(year) + "," + urlEncode(id);
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/gomovies_s.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/gomovies_s_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
foreach($videos as $video) {
  ...
  $id_t="";
   ...
   $link2=$host."/scripts/filme/php/gomovies_s_ep.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
   if ($title) {
     echo '
     <item>
