...
{
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/tvhub_add.php?mod=delete*" + getItemInfo(getFocusItemIndex(),"link1") + "*" + getItemInfo(getFocusItemIndex(),"title1");
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>tvhub - seriale favorite</title>
...
foreach ($arr as $key => $val) {
  ...
  $link = $host."/scripts/filme/php/vezionline_ep.php?file=".$l.",".urlencode(str_replace(",","^",$title));
  $link=$host."/scripts/filme/php/serialenoi_online_ep.php?file=".urlencode($l).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
