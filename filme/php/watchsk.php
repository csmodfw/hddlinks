...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/watchsk_add.php?mod=add," + urlEncode(movie) + "," + tit + "," + urlEncode(img) + "," + urlEncode(year) + "," + urlEncode(id);
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
tit=getItemInfo(getFocusItemIndex(),"tit");
showIdle();
movie_info="http://127.0.0.1/cgi-bin/scripts/filme/php/watchsk_det.php?file=" + movie+ "," + tit;
...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/watchsk.php?page=2,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/watchsk_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
if ($tip=="search")
   $requestLink = "http://watchseries.sk/search?s=".str_replace(" ","+",$link);
   $requestLink = "http://watchseries.fi/search?s=".str_replace(" ","+",$link);
...
foreach($videos as $video) {
  ...
  $t1 = explode('videoHname title">', $video);
  $t2 = explode('<', $t1[1]);
  $title11 = $t2[0];
  ...
  //$id_t=$id1;
  $id_t="";
   $link2=$host."/scripts/filme/php/watchsk_ep.php?file=".urlencode($link1).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
   if ($title) {
     echo '
     <item>
