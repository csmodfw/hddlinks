...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/putlockers_s_add.php?mod=add," + urlEncode(movie) + "," + urlEncode(tit) + "," + urlEncode(img) + "," + urlEncode(year) + "," + urlEncode(id);
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
tit=getItemInfo(getFocusItemIndex(),"tit");
showIdle();
movie_info="http://127.0.0.1/cgi-bin/scripts/filme/php/putlockers_f_det.php?file=" + movie+ "," + urlEncode(tit);
...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/putlockers_s.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/putlockers_s_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
*/
//echo $html;
//http://putlockers.is/tv
//http://putlockers.is/search/search.php?q=star%20trek&sort=views&page=1
if ($tip=="search")
   $requestLink = "http://putlockers.ch/search/search.php?q=".str_replace(" ","+",$link)."&search_section=2&page=".$page;
else
   $requestLink = "http://putlockers.ch//tv/".$page."";
...
//$cookie="D:/";
  ...
  curl_setopt ($ch, CURLOPT_REFERER, "http://putlockers.ch");
...
//print_r ($m);
 $videos = explode('padding-bottom: 10px;">', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $t2 = explode('"', $t1[1]);
  $title11 = $t2[0];
  $title11=trim(preg_replace("/\(\d+\)/i","",$title11));
  $id1="";
  ...
  //$id_t=$id1;
  $id_t="";
   $link2=$host."/scripts/filme/php/putlockers_s_ep.php?file=".urlencode($link1).",".urlencode($title).",".$id1.",".$id_t.",series,".urlencode($image);
   if ($title) {
     echo '
     <item>
     <title>'.$title.'</title>
