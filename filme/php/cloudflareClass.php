	...
	*/
	...
	public static function bypass($siteLink) {
		...
		if(!$cfClearanceCookie) {
			...
			// clear cookie log
			if (file_exists("/tmp/cookies.txt")) unlink('/tmp/cookies.txt');
	...
	private static function getCookie($siteNetLoc) {
		...
		$cfSessionFile = '/tmp/'.md5($siteNetLoc);
	...
	*/
	private static function getPage($url, $referer, $headers = array()){
		...
		if($curlResource = curl_init($url)){
        		...
        		// session cookies
        		curl_setopt($curlResource, CURLOPT_COOKIEJAR,  '/tmp/cookies.txt');
        		curl_setopt($curlResource, CURLOPT_COOKIEFILE, '/tmp/cookies.txt');
        		// return settings
