...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/pix_add.php?mod=del," + urlEncode(movie) + ",,,,";
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>hdeuropix - favorite</title>
...
foreach ($arr as $key => $val) {
  ...
  $image1=$image;
   $link2=$host."/scripts/filme/php/pix_sez.php?file=".urlencode($link).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
