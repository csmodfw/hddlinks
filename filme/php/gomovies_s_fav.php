...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/gomovies_s_add.php?mod=del," + urlEncode(movie) + ",,,,";
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
if (file_exists("/data"))
  $f= "/data/gomovies_s.dat";
else
  $f="/usr/local/etc/gomovies_s.dat";
if (file_exists($f)) {
...
foreach ($arr as $key => $val) {
  ...
  //$id1=substr(strrchr($link, "-"), 1);
   ...
   $link2=$host."/scripts/filme/php/gomovies_s_ep.php?file=".urlencode($link).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",series,".urlencode($image);
   if ($title) {
     echo '
     <item>
