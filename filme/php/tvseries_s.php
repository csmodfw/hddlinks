...
{
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/tvseries_s_add.php?mod=add*" + getItemInfo(getFocusItemIndex(),"link1") + "*" + getItemInfo(getFocusItemIndex(),"tit");
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/filme/php/tvseries_s.php?page=1,search,";?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
<fs>
...
';
  ...
  $link1 = $host."/scripts/filme/php/tvseries_s_fav.php";
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link1.'</link>
...
foreach($videos as $video) {
  ...
  $image = "";
   ...
   if ($title && strpos($link1,"/tv") !== false) {
     echo '
     <item>
     <title>'.str_replace("&","&amp;",str_replace("&amp;","&",$title)).'</title>
     <link>'.$link2.'</link>
