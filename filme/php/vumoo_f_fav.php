...
id=getItemInfo(getFocusItemIndex(),"id");
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/vumoo_f_add.php?mod=del," + urlEncode(movie) + ",,,,";
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>vumoo filme - favorite</title>
...
foreach ($arr as $key => $val) {
  ...
  $id1=substr(strrchr($link, "-"), 1);
   ...
   $link2=$host."/scripts/filme/php/vumoo_f_link.php?file=".urlencode($link).",".urlencode(str_replace(",","^",$title)).",".$id1.",".$id_t.",movie,".urlencode($image);
   if ($title) {
     echo '
     <item>
