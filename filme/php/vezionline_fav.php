...
{
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/vezionline_add.php?mod=delete*" + getItemInfo(getFocusItemIndex(),"link1") + "*" + getItemInfo(getFocusItemIndex(),"title1");
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>vezionline - seriale favorite</title>
...
if (file_exists("/data"))
  $f= "/data/vezionline.dat";
else
  $f="/usr/local/etc/vezionline.dat";
if (file_exists($f)) {
...
foreach ($arr as $key => $val) {
  ...
  $l=str_replace("vezionline.org","cinemahd.eu",$l);
