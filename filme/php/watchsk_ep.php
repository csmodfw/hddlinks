...
//$cookie="D:/";
 ...
 $videos = explode('class="video-module-title">', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $h = curl_exec($ch);
  curl_close($ch);
  $sezons = explode('class="video_container"', $h);
  unset($sezons[0]);
  $sezons = array_values($sezons);
  foreach($sezons as $sezon) {
    $t1=explode('href="',$sezon);
    $t2=explode('"',$t1[1]);
    $link_ep=trim($t2[0]);
    $ep_tit=trim(str_between($sezon,'</b>','<'));
    $ep_tit=str_replace("?","",$ep_tit);
    $ep_tit=html_entity_decode($ep_tit,ENT_QUOTES,'UTF-8');
  //$t1=explode('>',$video);
  //season-1-episode-3
  preg_match("/season\/(\d+)\/episode\/(\d+)/",$link_ep,$m);
  //print_r ($m);
  $season=$m[1];
  $episod=$m[2];
   ...
   $title1=$ep_tit;
   $title=$ep_tit;
   $t1=explode('src="',$sezon);
   $t2=explode('"',$t1[1]);
   $image=$t2[0];
   $image1=$image;
   $link2=$host."/scripts/filme/php/watchsk_link.php?file=".urlencode($link_ep).",".urlencode(str_replace(",","^",$title1)).",".urlencode(str_replace(",","^",$series_title_no_year)).",".$id_t.",series,".urlencode($image);
   if ($video) {
     echo '
     <item>
